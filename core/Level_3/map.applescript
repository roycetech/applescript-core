(*
	...
	@Build:
		./scripts/build-lib.sh core/Level_3/map
*)
...
on newFromString(theText)
	...
	repeat with nextLine in mapLines
		...
		-- userDictionary's putValue(nextKey, userDictionary's __JoinList(rest of lineTokens, ":"))
		userDictionary's putValue(nextKey, nextValue)
	end repeat
...
(* @deprecated use the alias newFromRecord. *)
on fromRecord(sourceRecord)
	newFromRecord(sourceRecord)
end fromRecord
...
on newFromRecord(sourceRecord)
	...
	set objCDictionary to current application's NSDictionary's dictionaryWithDictionary:sourceRecord
	...
	repeat with theKey in allKeys
		...
		-- set nextValue to contents of a reference to nextValueRaw
		-- set nextValue to nextValueRaw
		if nextValue as text is equal to "msng" then -- Workaround, we could not detect the missing value from the way we instantiated the dictionary.
		...
		userDictionary's putValue(theKey as text, nextValue)
	end repeat
...
on new()
	script ASDictionary
		...
		on valueForKey(aKey) -- (object) as object or (missing value)
			...
			item keyValueIndex of __values
		end valueForKey
