...
on replaceText(filePath, substring, replacement)
	...
	-- if replacement contains "|" then set escapedReplacement to textUtil's replace(escapedReplacement, "|", "\\|")
	-- if replacement contains "*" then set escapedReplacement to textUtil's replace(escapedReplacement, "*", "\\*")
	-- if replacement contains "[" then set escapedReplacement to textUtil's replace(escapedReplacement, "[", "\\[")
	-- if replacement contains "]" then set escapedReplacement to textUtil's replace(escapedReplacement, "]", "\\]")
...
FROM=$(escape_search \"$old\")
TO=$(escape_replace \"$new\")

sed -i '' \"s|$FROM|$TO|g\" " & quotedFilePath
