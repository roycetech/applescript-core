(*
	...
	@Build:
		./scripts/build-lib.sh core/Level_2/dialog

	TODO:
		Remove speech dependency.
...
on new()
	...
	script DialogInstance
		...
		on showWarningWithTimeout(theTitle, message, timeoutSeconds)
			set titleWithTimeout to format {"{} (auto-close in {}s)", {theTitle, timeoutSeconds}}
			display dialog message with title titleWithTimeout with icon 2 buttons {"OK"} giving up after timeoutSeconds
		...
		on showChoicesWithTimeout(theTitle, message, choices as list, defaultChoice, timeoutSeconds)
			...
			set titleWithTimeout to format {"{} (auto-close in {}s)", {theTitle, timeoutSeconds}}
			display dialog message with title titleWithTimeout with icon 1 buttons choices default button defaultChoice giving up after timeoutSeconds
		...
		*)
		on confirmWarningWithTimeout(theTitle as text, yourMessage as text, timeoutSeconds)
			display alert theTitle message yourMessage as critical buttons {"Yes", "No"} default button "no" giving up after timeoutSeconds
	...
	decorator's decorateByName("DialogInstance")
end new
