...
on format(theString, theTokens)
	...
	set textList to ""
	set sanitizedToken to false
	set sanitizedSource to false
	set nextCleanItem to missing value
	set isSingleQuote to theString contains "'"
	
	repeat with nextItem in theTokens
		if nextItem contains "'" then
			set nextCleanItem to replace(nextItem, "'", "!")
			set sanitizedToken to true
			set isSingleQuote to true
			
		else if nextItem contains "\"" then
			set nextCleanItem to replace(nextItem, "\"", "!")
			set sanitizedToken to true
			...
			set nextCleanItem to nextItem
		end if
		if textList is not "" then set textList to textList & ", "
		set textList to textList & "\"" & nextCleanItem & "\""
	end repeat
	
	-- logger's debugf("theString: {}", theString) would trigger stack overflow.
	if theString contains "'" then
		set theStringClean to replace(theString, "'", "!")
		set sanitizedSource to true
		
	else if theString contains "\"" then
		set theStringClean to replace(theString, "\"", "\\\"")
		set sanitizedSource to true
	end if
	
	if not sanitizedSource then set theStringClean to theString
	
	if nextCleanItem is not missing value then
		set theCommand to "/usr/bin/python3 -c 'print(\"\"\"" & theStringClean & "\"\"\".format(" & textList & "))'"
		...
		if sanitizedSource or sanitizedToken then
			set theQuote to "\""
			if isSingleQuote then set theQuote to "'"
			return replace(shellResult, "!", theQuote)
		end if
	...
	set builtString to theString
	repeat with nextToken in theTokens
		-- if nextToken contains "/" then set nextToken to replace(nextToken, "/", "\\/")
		
		-- set builtString to do shell script "echo '" & builtString & "' | sed 's/{}/" & nextToken & "/'"
		-- set builtString to do shell script "echo '" & builtString & "' | sed 's/{}/" & nextToken & "/'"
		set builtString to replaceFirst(builtString, "{}", nextToken as text) -- more reliable than sed, sed conflicts.
	end repeat
	builtString
end format
