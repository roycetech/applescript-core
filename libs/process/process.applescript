(*
	...
	Notes:
		...
		Under Level 3 scripts due to the need to use a retry operation.
	...
	TODO:
		Move out the Dock dependency.

	@Project:
		applescript-core

	@Build:
		./scripts/build-lib.sh libs/process/process
...
on new(pProcessName)
	...
	script ProcessInstance
		...
		*)
		...
		on waitActivate()
			...
			if waitWindowResult is not missing value then return true
	...
	set decorator to decoratorLib's new(staticDecoratedInstance)
	decorator's decorateByName("ProcessInstance")
end new
