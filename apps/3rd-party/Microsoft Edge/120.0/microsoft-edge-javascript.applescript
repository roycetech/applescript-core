(*
	...
	Testing Template:
		...
		property msedge : msedgeLib's new()
...
on spotCheck()
	...
	set msedge to msedgeLib's new()
	set msedgeTab to msedge's getFrontTab()
	set msedgeTab to decorate(msedgeTab)
	...
	msedgeTab's focus()
	msedgeTab's waitForPageLoad()
	
	if caseIndex is 1 then
		-- logger's infof("Selector Exists: {}",
		log msedgeTab's selectorExists("#iam_user_radio_button")
		log msedgeTab's selectorExists("#account")
		-- )
		
	else if caseIndex is 2 then
		set jsResult to msedgeTab's linkTextVisible("Learn more")
		...
		assertThat of std given condition:msedgeTab's selectorExists(".alert-danger") is false, messageOnFail:"Failed spot check"
		
		assertThat of std given condition:msedgeTab's selectorExists(".unit-wrapper") is true, messageOnFail:"Failed spot check"
		logger's info("Passed.")
...
on decorate(msedgeTab)
	...
	set findRunMax of MicrosoftEdgeJavaScriptDecorator to configSystem's getValue("FIND_RETRY_MAX")
	set findRetrySleep of MicrosoftEdgeJavaScriptDecorator to configSystem's getValue("FIND_RETRY_SLEEP")
	MicrosoftEdgeJavaScriptDecorator
end decorate
