(*
	...
	@Build:
		./scripts/build-lib.sh 'apps/3rd-party/Opera/110.0/dec-opera-tab-finder'

	@Created: Saturday, March 16, 2024 at 11:13:08 AM
	@Last Modified: 2024-12-31 19:32:56
	@Change Logs:
*)
...
on spotCheck()
	...
	set sutLib to script "core/opera"
...
on decorate(mainScript)
	...
	script operaFinderDecorator
		property parent : mainScript

		on getTabCount()
			if running of application "Opera" is false then return 0
		...
		*)
		on findTabWithName(targetName)
			if running of application "Opera" is false then return missing value

			tell application "Opera"
				repeat with nextWindow in windows
					try
						if title of active tab of nextWindow is equal to targetName then
							return operaTabLib's new(id of nextWindow, active tab index of nextWindow, me)
						end if

						set matchedTab to (first tab of nextWindow whose title is equal to targetName)
						return operaTabLib's new(id of nextWindow, index of matchedTab as integer, me)
					end try
				end repeat
			end tell
			...
			tell application "Opera"
				tell front window
					if URL of active tab contains urlSubstring then
						return operaTabLib's new(its id, active tab index as integer, me)
					end if
				end tell
