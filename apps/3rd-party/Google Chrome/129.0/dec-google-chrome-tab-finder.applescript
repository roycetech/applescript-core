				...
				repeat with nextWindow in windows
					try
						if title of active tab of nextWindow is equal to targetName then
							return googleChromeTabLib's new(id of nextWindow, active tab index of nextWindow, me)
						end if
						
						set matchedTab to (first tab of nextWindow whose title is equal to targetName)
						return googleChromeTabLib's new(id of nextWindow, index of matchedTab as integer, me)
					end try
				end repeat
			end tell
			...
			tell application "Google Chrome"
				tell front window
					if URL of active tab contains urlSubstring then
						return googleChromeTabLib's new(its id, active tab index as integer, me)
					end if
				end tell
