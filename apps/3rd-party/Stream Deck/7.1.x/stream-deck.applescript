(* 
	...
	WARNING: This app is likely not a native macOS app so UI interactions via script is unreliable. Use cliclick to workaround this.
...
property DEVICE_DEFAULT : DEVICE_XL
...
on spotCheck()
	...
	logger's infof("Current page: {}", sut's getCurrentPage())
	...
	*)
	...
	if caseIndex is 1 then
		...
		set sutPage to "unicorn"
		set sutPage to "pinned"
		set sutPage to 1
		set sutPage to 99
		logger's debugf("sutPage: {}", sutPage)
		
		sut's switchPage(sutPage)
		
	else if caseIndex is 7 then
		sut's shrinkEditorWindow()
	...
	script StreamDeckInstance
		on getFirstProfileName()
			...
			tell application "System Events" to tell process "Stream Deck"
				try
					return title of first menu item of menu 1 of menu item DEVICE_DEFAULT of menu 1 of menu bar 2
				end try
			end tell
			
			missing value
		end getFirstProfileName
		
		
		on getLastProfileName()
			tell application "System Events" to tell process "Stream Deck"
				try
					return title of last menu item of menu 1 of menu item DEVICE_DEFAULT of menu 1 of menu bar 2
				end try
			end tell
			
			missing value
		end getLastProfileName
		...
		*)
		on getCurrentPage()
			...
			if pageDescription contains "pin" then return "pinned"
			last word of pageDescription
		end getCurrentPage
		
		
		on switchPage(newPage)
			if newPage is 0 or newPage is "pinned" then
				tell application "System Events" to tell process "Stream Deck"
			...
			tell application "System Events" to tell process "Stream Deck"
				set frontmost to true
				try
					-- click (first checkbox of group 1 of splitter group 1 of group 1 of window "Stream Deck" whose description ends with newPage)
					first checkbox of group 1 of splitter group 1 of group 1 of window "Stream Deck" whose description ends with newPage
					lclick of cliclick at result
				end try
		...
		(*
			Set to a predictable size so that UI automation that rely on 
			coordinates can be performed more reliably.

			Duplicate of dec-stream-deck-button#resizeEditorWindowToMinimum().
		*)
		on shrinkEditorWindow()
			if running of application "Elgato Stream Deck" is false then return
			if not isEditorWindowPresent() then return
			
			tell application "System Events" to tell process "Stream Deck"
				try
					set size of window "Stream Deck" to {EDITOR_MINIMUM_WIDTH, EDITOR_MINIMUM_HEIGHT}
				end try
			end tell
		end shrinkEditorWindow
		...
		(*
			Used by the Keyboard Maestro macro to propagate the shared folder to all profiles.
		*)
		on focusProcess()
			if running of application "Elgato Stream Deck" is false then return
		...
		on isUsbConnected()
			try
				...
				return (do shell script SHELL_USB_CHECK) is not ""
			end try
