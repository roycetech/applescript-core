(*
	...
	@Build:
		./scripts/build-lib.sh 'apps/3rd-party/Stream Deck/7.1/dec-stream-deck-settings'

	@Created: Wed, Oct 01, 2025 at 01:04:40 PM
	@Last Modified: Wed, Oct 01, 2025 at 01:04:40 PM
	
	@Change Logs:
		Sat, Feb 28, 2026, at 01:01:49 PM
			- Added #bringSettingsWindowToFront()
			- Fixed UI hierarchy change 
...
on spotCheck()
	...
	set cases to listUtil's splitByLine("
		...
		Manual: Settings window - switch settings tab
	")
...
on decorate(mainScript)
	...
	script StreamDeckSettingsDecorator
		...
		on showSettingsWindow()
			...
			if isSettingsWindowPresent() then
		...
		on bringSettingsWindowToFront()
			tell application "System Events" to tell process "Stream Deck"
				perform action "AXRaise" of window "Preferences"
			end tell
		end bringSettingsWindowToFront
