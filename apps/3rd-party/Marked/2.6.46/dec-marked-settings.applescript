	...
	script MarkedSettingsDecorator
		...
		on showSettings()
			if isSettingsWindowPresent() then
				logger's debug("Settings window detected, exiting...")
				return
			end if
		...
		on getSettingsWindow()
			...
			tell application "System Events" to tell process "Marked 2"
				...
				*)
				...
				repeat with nextWindow in currentWindows
					if the title of nextWindow does not end with "%)" and the title of the nextWindow does not end with ".md" then return nextWindow
