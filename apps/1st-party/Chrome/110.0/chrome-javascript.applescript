(*
	...
	Testing Template:
		...
		property chrome : chromeLib's new()
	...
	@Build:
		./scripts/build-lib.sh apps/1st-party/Chrome/110.0/chrome-javascript
...
on spotCheck()
	...
	set chromeTab to decorate(result)
	...
	if caseIndex is 1 then
		...
		log chromeTab's selectorExists("#iam_user_radio_button")
		log chromeTab's selectorExists("#account")
		-- )

	else if caseIndex is 2 then
		set jsResult to chromeTab's linkTextVisible("Learn more")
		...
		assertThat of std given condition:chromeTab's selectorExists(".alert-danger") is false, messageOnFail:"Failed spot check"

		assertThat of std given condition:chromeTab's selectorExists(".unit-wrapper") is true, messageOnFail:"Failed spot check"
		logger's info("Passed.")
...
on decorate(chromeTab)
	...
	set findRunMax of ChromeJavaScriptDecorator to configSystem's getValue("FIND_RETRY_MAX")
	set findRetrySleep of ChromeJavaScriptDecorator to configSystem's getValue("FIND_RETRY_SLEEP")
