...
property scriptName : "cliclick" -- The name of the script to be tested
...
script |cliclick._formatCoordinates tests|
	property parent : TestSet(me)
	...
	script |Left positive|
		property parent : UnitTest(me)
		assertEqual("1,=-1", sut's _formatCoordinates(1, -1))
	end script

	script |Right positive|
		property parent : UnitTest(me)
		assertEqual("=-1,1", sut's _formatCoordinates(-1, 1))
	end script

	script |Both positive|
		property parent : UnitTest(me)
		assertEqual("1,2", sut's _formatCoordinates(1, 2))
	end script

	script |Both negative|
		property parent : UnitTest(me)
		assertEqual("=-1,=-2", sut's _formatCoordinates(-1, -2))
	end script
end script
