...
script |swap tests|
	...
	script |Missing list|
		property parent : UnitTest(me)
		set parameter to missing value
		sutScript's swap(parameter, 1, 2)
		assertMissing(parameter)
	end script
	
	script |Target out of bounds for source index|
		property parent : UnitTest(me)
		script Lambda
			sutScript's swap({1, 2}, 99, 1)
		end script
		shouldRaise(sutScript's ERROR_OUT_OF_BOUNDS, Lambda, "Expected error was not thrown")
	end script
	
	script |Target out of bounds for target index|
		property parent : UnitTest(me)
		script Lambda
			sutScript's swap({1, 2}, 1, 99)
		end script
		shouldRaise(sutScript's ERROR_OUT_OF_BOUNDS, Lambda, "Expected error was not thrown")
	end script
	
	script |Missing source index|
		property parent : UnitTest(me)
		script Lambda
			sutScript's swap({1, 2}, 99, missing value)
		end script
		shouldRaise(sutScript's ERROR_OUT_OF_BOUNDS, Lambda, "Expected error was not thrown")
	end script
	
	script |Missing target index|
		property parent : UnitTest(me)
		script Lambda
			sutScript's swap({1, 2}, missing value, 99)
		end script
		shouldRaise(sutScript's ERROR_OUT_OF_BOUNDS, Lambda, "Expected error was not thrown")
	end script
	...
	script |Same Index|
		property parent : UnitTest(me)
		set parameter to {1, 2, 3}
		sutScript's swap(parameter, 2, 2)
		assertEqual({1, 2, 3}, parameter)
	end script
	
	script |Happy|
		property parent : UnitTest(me)
		set parameter to {1, 2, 3}
		sutScript's swap(parameter, 1, 3)
		assertEqual({3, 2, 1}, parameter)
	end script
	
end script


script |moveElement tests|
	property parent : TestSet(me)
	
	script |Missing list|
		property parent : UnitTest(me)
		set parameter to missing value
		sutScript's moveElement(parameter, 1, 2)
		assertMissing(parameter)
	end script
	...
	script |Target out of bounds for source index|
		property parent : UnitTest(me)
		script Lambda
			sutScript's moveElement({1, 2}, 99, 1)
		end script
		shouldRaise(sutScript's ERROR_OUT_OF_BOUNDS, Lambda, "Expected error was not thrown")
	end script
	...
	script |Target out of bounds for target index|
		property parent : UnitTest(me)
		script Lambda
			sutScript's moveElement({1, 2}, 1, 99)
		end script
		shouldRaise(sutScript's ERROR_OUT_OF_BOUNDS, Lambda, "Expected error was not thrown")
	end script
	...
	script |Same Index|
		property parent : UnitTest(me)
		assertEqual({1, 2, 3}, sutScript's moveElement({1, 2, 3}, 2, 2))
	end script
	
	script |Move forward|
		property parent : UnitTest(me)
		assertEqual({2, 1, 3}, sutScript's moveElement({1, 2, 3}, 1, 2))
	end script
	
	script |Move backward|
		property parent : UnitTest(me)
		assertEqual({1, 3, 2}, sutScript's moveElement({1, 2, 3}, 3, 2))
	end script
end script